...
import litellm
from experiment_utils import find_latest_files, load_json_data, save_evaluation_results
from scoring import calculate_comprehensive_similarity, calculate_exact_muscle_metrics
...
VALID_MUSCLES = {
    "abdominal", "bicep", "calf", "chest", "forearm - inner", "forearm - outer",
    "glute", "hamstring", "lat", "lower back", "oblique", "quad", 
    "rotator cuff - back", "rotator cuff - front", "shoulder - back", 
    "shoulder - front", "shoulder - side", "thigh - inner", "thigh - outer",
    "trap", "tricep"
}
...
def evaluate_exercise_name(expected_name: str, actual_name: str, embedding_client: EmbeddingClient) -> Dict[str, Any]:
    """Evaluate exercise name using semantic and lexical similarity."""
    if not actual_name or actual_name == "Unknown":
        return {
            "semantic_similarity": 0.0,
            "lexical_similarity": {
                "ratio": 0.0, "partial_ratio": 0.0, "token_sort_ratio": 0.0, "token_set_ratio": 0.0
            },
            "combined_score": 0.0,
    ...
    try:
        return calculate_comprehensive_similarity(expected_name, actual_name, embedding_client)
...
def evaluate_exercise_steps(expected_steps: List[str], actual_steps: List[str], embedding_client: EmbeddingClient) -> Dict[str, Any]:
    ...
    try:
        ...
        step_count_accuracy = min(actual_count / expected_count, 1.0) if expected_count > 0 else 0.0
        
        # Semantic similarity between concatenated steps
        expected_text = " ".join(expected_steps)
        actual_text = " ".join(actual_steps)
        
        similarity_result = calculate_comprehensive_similarity(expected_text, actual_text, embedding_client)
        
        # Combined score (weight step count and semantic similarity)
        combined_score = (step_count_accuracy * 0.3) + (similarity_result["combined_score"] * 0.7)
...
def evaluate_exercise_muscles(expected_muscles: Dict[str, List[str]], actual_muscles: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Calculate metrics using existing scoring function
        result = calculate_exact_muscle_metrics(
            expected_primary, expected_secondary,
            actual_primary, actual_secondary
        )
        
        # Add validation info
        result["invalid_muscles"] = invalid_muscles
        result["error"] = None
    ...
    for result in results:
        if not result["success"]:
            ...
            evaluation = {
                "name_evaluation": {
                    "semantic_similarity": 0.0,
                    "lexical_similarity": {"ratio": 0.0, "partial_ratio": 0.0, "token_sort_ratio": 0.0, "token_set_ratio": 0.0},
                    "combined_score": 0.0,
            ...
            overall_score = (name_score * 0.30) + (steps_score * 0.35) + (muscles_score * 0.35)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate end-to-end strength flow results")
    parser.add_argument("--input", type=str, help="Path to results JSON file to evaluate")
    ...
    if args.output:
        ...
        output_file = save_evaluation_results(evaluated_results, input_file, "end_to_end_strength_flow")
        print(f"Evaluation results saved to {output_file}")
    
    # Print summary
    successful_evals = [r for r in evaluated_results if r["success"] and not r["evaluation"]["error"]]
    
    if successful_evals:
        # Calculate averages
        avg_overall = sum(r["evaluation"]["overall_score"] for r in successful_evals) / len(successful_evals)
        avg_name = sum(r["evaluation"]["name_evaluation"]["combined_score"] for r in successful_evals) / len(successful_evals)
        avg_steps = sum(r["evaluation"]["steps_evaluation"]["combined_score"] for r in successful_evals) / len(successful_evals)
        avg_muscles = sum(r["evaluation"]["muscles_evaluation"]["combined_score"] for r in successful_evals) / len(successful_evals)
        
        print(f"\nEvaluation Summary:")
        print(f"Evaluated: {len(successful_evals)}/{len(evaluated_results)} successful results")
