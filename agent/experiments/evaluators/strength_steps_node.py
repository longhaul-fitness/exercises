...
def run_flexibility_steps_evaluator(
    ...
    """
    Evaluate StrengthStepsNode results using semantic step matching.
    ...
    if input_path:
        ...
        results_files = find_latest_files("strength_steps_node", "runs", test_case_id)
...
def main():
    """CLI interface for running the strength steps node evaluator."""
    import argparse

    parser = argparse.ArgumentParser(description="Evaluate StrengthStepsNode results")
    parser.add_argument(
    ...
    if args.stdout:
        ...
        filepath = save_evaluation_results(
            evaluated_results, source_filename, "strength_steps_node"
        )
