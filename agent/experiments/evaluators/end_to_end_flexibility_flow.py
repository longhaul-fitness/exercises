"""
End-to-end evaluator for flexibility flow results.

Evaluates the complete output (name + steps + muscles) from end-to-end flexibility
...
def evaluate_end_to_end_results(
    ...
    """
    Evaluate end-to-end flexibility flow results.
...
def main():
    """CLI interface for evaluating end-to-end flexibility flow results."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Evaluate end-to-end flexibility flow results"
    )
    parser.add_argument(
    ...
    if args.input:
        ...
        # Find the latest end-to-end flexibility flow results
        latest_files = find_latest_files("end_to_end_flexibility_flow", "runs")
        if not latest_files:
            print("No end-to-end flexibility flow results found")
            return
    ...
    if args.output:
        ...
        output_file = save_evaluation_results(
            evaluated_results, input_file, "end_to_end_flexibility_flow"
        )
        print(f"Evaluation results saved to {output_file}")
