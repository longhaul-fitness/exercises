...
def run_flexibility_name_evaluator(
    ...
    """
    Evaluate StrengthNameNode results using fuzzy F1 scoring.
    ...
    if input_path:
        ...
        results_files = find_latest_files("strength_name_node", "runs", test_case_id)
...
def main():
    """CLI interface for running the strength name node evaluator."""
    import argparse

    parser = argparse.ArgumentParser(description="Evaluate StrengthNameNode results")
    parser.add_argument(
    ...
    if args.stdout:
        ...
        filepath = save_evaluation_results(
            evaluated_results, source_filename, "strength_name_node"
        )
