...
from nodes import StrengthStepsNode


def prepare_steps_node_input(test_case: Dict[str, Any], model: str) -> Dict[str, Any]:
    """Prepare input data for StrengthStepsNode."""
    return {
...
def run_strength_steps_node(
    models: Optional[List[str]] = None,
    test_case_id: Optional[str] = None,
    data_path: Optional[str] = None,
) -> List[Dict[str, Any]]:
    """
    Run StrengthStepsNode with specified models and test cases.
    ...
    return run_node_experiments(
        node_class=StrengthStepsNode,
        expected_field="steps",
...
def main():
    """CLI interface for running the strength steps node experiments."""
    import argparse

    parser = argparse.ArgumentParser(description="Run StrengthStepsNode experiments")
    ...
    # Run the experiments
    results = run_strength_steps_node(
    ...
    if args.output:
        ...
        filepath = save_json_results(
            results, models or DEFAULT_MODELS, "strength_steps_node"
        )
