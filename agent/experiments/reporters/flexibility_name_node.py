...
def process_evaluation_results(
    ...
    Returns:
        ...
        - model_metrics: {model_name: {"cost": median_cost, "quality": median_quality}}
    ...
    for result in results:
        ...
        # Extract cost and quality
        try:
            cost = float(result["metadata"]["cost"])
            quality = result["evaluation"]["combined_score"]
...
def create_pareto_chart(
    ...
    for i, model in enumerate(models):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                + "Quality (Combined Score): %{x:.4f}<br>"
                + "Cost: %{y:.10f}<br>"
                + "<extra></extra>",
            )
        )
    ...
    fig.update_layout(
        ...
        xaxis_title="Quality (Combined Score)",
        yaxis_title="Cost",
        width=800,
        height=600,
        showlegend=True,
...
def generate_chart_title(source_filename: str) -> str:
    ...
    return f"FlexibilityNameNode: Cost vs Quality Pareto Analysis ({date_str})"
...
def save_chart(fig: go.Figure, source_filename: str) -> str:
    ...
    results_dir = os.path.join(
        os.path.dirname(__file__), "../results/flexibility_name_node/reports"
    )
...
def run_flexibility_name_reporter(
    input_path: Optional[str] = None, use_stdin: bool = False
) -> Tuple[go.Figure, str]:
    """
    Generate Pareto optimization report for FlexibilityNameNode evaluation results.
    ...
    if use_stdin:
        ...
        eval_files = find_latest_files("flexibility_name_node", "evaluations")
        if not eval_files:
...
def main():
    """CLI interface for running the flexibility name node reporter."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Generate Pareto optimization report for FlexibilityNameNode"
    )
    parser.add_argument(
    ...
    try:
        # Generate report
        fig, source_filename = run_flexibility_name_reporter(
