...
def generate_chart_title(source_filename: str) -> str:
    ...
    return f"StrengthNameNode: Cost vs Quality Pareto Analysis ({date_str})"
...
def save_chart(fig: go.Figure, source_filename: str) -> str:
    ...
    results_dir = os.path.join(
        os.path.dirname(__file__), "../results/strength_name_node/reports"
    )
...
def run_flexibility_name_reporter(
    ...
    """
    Generate Pareto optimization report for StrengthNameNode evaluation results.
    ...
    if use_stdin:
        ...
        eval_files = find_latest_files("strength_name_node", "evaluations")
        if not eval_files:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Pareto optimization report for StrengthNameNode"
    )
    parser.add_argument(
