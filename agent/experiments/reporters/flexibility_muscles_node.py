...
def process_evaluation_results(
    ...
    Returns:
        Tuple of (model_metrics, error_counts, invalid_muscle_counts) where:
        - model_metrics: {model_name: {"cost": median_cost, "quality": median_f1_score}}
        - error_counts: {model_name: error_count}
        - invalid_muscle_counts: {model_name: {"primary": count, "secondary": count}}
    """
    model_data = {}
    error_counts = {}
    invalid_muscle_counts = {}
    ...
    for result in results:
        ...
        if model not in model_data:
            ...
            invalid_muscle_counts[model] = {"primary": 0, "secondary": 0}
        ...
        try:
            ...
            # Count invalid muscles
            invalid_muscles = result["evaluation"]["invalid_muscles"]
            invalid_muscle_counts[model]["primary"] += len(invalid_muscles["primary"])
            invalid_muscle_counts[model]["secondary"] += len(
                invalid_muscles["secondary"]
            )
    ...
    return model_metrics, error_counts, invalid_muscle_counts
...
def generate_chart_title(source_filename: str) -> str:
    ...
    return f"FlexibilityMusclesNode: Cost vs Quality Pareto Analysis ({date_str})"
...
def save_chart(fig: go.Figure, source_filename: str) -> str:
    ...
    results_dir = os.path.join(
        os.path.dirname(__file__), "../results/flexibility_muscles_node/reports"
    )
...
def run_flexibility_muscles_reporter(
    input_path: Optional[str] = None, use_stdin: bool = False
) -> Tuple[go.Figure, str]:
    """
    Generate Pareto optimization report for FlexibilityMusclesNode evaluation results.
    ...
    if use_stdin:
        ...
        eval_files = find_latest_files("flexibility_muscles_node", "evaluations")
        if not eval_files:
    ...
    # Process results
    model_metrics, error_counts, invalid_muscle_counts = process_evaluation_results(
        results
    )
    ...
    # Print summary of errors and invalid muscles
    ...
    total_invalid_muscles = sum(
        counts["primary"] + counts["secondary"]
        for counts in invalid_muscle_counts.values()
    )
    if total_invalid_muscles > 0:
        print("\nInvalid muscle usage detected:")
        for model, counts in invalid_muscle_counts.items():
            total_invalid = counts["primary"] + counts["secondary"]
            if total_invalid > 0:
                print(
                    f"  {model}: {counts['primary']} primary, {counts['secondary']} secondary"
                )
...
def main():
    """CLI interface for running the flexibility muscles node reporter."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Generate Pareto optimization report for FlexibilityMusclesNode"
    )
    parser.add_argument(
    ...
    try:
        # Generate report
        fig, source_filename = run_flexibility_muscles_reporter(
